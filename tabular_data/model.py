...
def preprocessing_model_tabular_data(df):
    ...
    return pd.DataFrame(preprocessed_data_X), y_encoded

#Encode the target and model with the preprocess data
def modeling(df):

    model_path = os.path.join(ROOT_PATH, 'tabular_data', 'models', 'mvp_model.pkl')
    model = LogisticRegression()
    model.fit(preprocessing_model_tabular_data(df)[0], preprocessing_model_tabular_data(df)[1])

    # Export the model as a pickle file, we might need to create a models folder on the taabolar data folder
    with open(model_path, 'wb') as file:
        pickle.dump(model, file)

def prediction_tabular_data(gender,age, smoking,yellow_fingers, anxiety, peer_pressure, chronic_disease, fatigue, allergy, wheezing, alcoholl, coughing, shortness_breath,swalloging_difficulty, chest_pain):

    tabular_to_predict = pd.DataFrame([[gender,smoking,yellow_fingers, anxiety, peer_pressure, chronic_disease, fatigue, allergy, wheezing, alcoholl, coughing, shortness_breath,swalloging_difficulty, chest_pain, age]], columns=['GENDER', 'SMOKING', 'YELLOW_FINGERS', 'ANXIETY',
       'PEER_PRESSURE', 'CHRONIC DISEASE', 'FATIGUE ', 'ALLERGY ', 'WHEEZING',
       'ALCOHOL CONSUMING', 'COUGHING', 'SHORTNESS OF BREATH',
    ...
    tabular_data_preprocessed = preprocessing_model_tabular_data(tabular_to_predict)

    #PREDICT
    prediction = model.predict(tabular_data_preprocessed[0])
